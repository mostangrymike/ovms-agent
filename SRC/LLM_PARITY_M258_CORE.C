...
int llm_mcp_run_result_base(const char *config,
...
#define llm_mcp_exec_all_text llm_mcp_exec_all_base
#define llm_mcp_exec_transport_text llm_mcp_exec_transport_base
#define llm_mcp_run_result llm_mcp_run_result_base
#define llm_mcp_agent_call llm_mcp_agent_call_base
#define llm_show_mcp_execute llm_show_mcp_execute_base
#define llm_mcp_file_bridge_execute llm_mcp_bridge_file_base
#define llm_mcp_bridge_executor llm_mcp_bridge_exec_base
#define llm_mcp_http_bridge_executor llm_mcp_http_exec_base
#define llm_mcp_sse_bridge_executor llm_mcp_sse_exec_base
#include "LLM_PARITY_CORE.INC"
#undef llm_mcp_exec_all_text
#undef llm_mcp_exec_transport_text
#undef llm_mcp_run_result
#undef llm_mcp_agent_call
#undef llm_show_mcp_execute
#undef llm_mcp_file_bridge_execute
#undef llm_mcp_bridge_executor
#undef llm_mcp_http_bridge_executor
#undef llm_mcp_sse_bridge_executor
...
{
    FILE *file;
    char command[LLM_MCP_BRIDGE_COMMAND_MAX];
    char line[512];
    size_t used;
    ...
    if (!llm_mcp_stdio_target_valid(bridge) || transport == NULL ||
    ...
    file = fopen(LLM_MCP_BRIDGE_REQUEST_FILE, "w");
    if (file == NULL) return 0;

    if (strcmp(transport, "stdio") == 0) {
        if (fprintf(file, "tool=%s\narguments=%s\n", tool, arguments) < 0) {
            (void)fclose(file);
            (void)remove(LLM_MCP_BRIDGE_REQUEST_FILE);
            return 0;
        }
    } else {
        if (fprintf(file,
    ...
    (void)remove(LLM_MCP_BRIDGE_RESPONSE_FILE);
    written = snprintf(command, sizeof(command), "%s %s %s", bridge,
                       LLM_MCP_BRIDGE_REQUEST_FILE,
                       LLM_MCP_BRIDGE_RESPONSE_FILE);
    ...
    file = fopen(LLM_MCP_BRIDGE_RESPONSE_FILE, "r");
    if (file == NULL) {
        (void)m258_bridge_note(result, result_size, status,
                               timeout_seconds, output_limit,
                               "response_missing");
...
{
    (void)context;
    if (!llm_mcp_stdio_target_valid(target)) return 0;
                              ...
                              void *context)
{
    const char *bridge;
    (void)context;
    if (!llm_mcp_http_target_valid(target)) return 0;
    bridge = getenv("OVMS_AGENT_MCP_HTTP_BRIDGE");
    if (!llm_mcp_stdio_target_valid(bridge)) return 0;
                             ...
                             size_t detail_size)
{
    ...
    if (strcmp(llm_approval_name(), "full") != 0) return 1;

    cursor = arguments;
    if (!llm_mcp_call_token(&cursor, server, sizeof(server)) ||
        !llm_mcp_call_token(&cursor, tool, sizeof(tool))) return 1;

    count = llm_mcp_parse(config, servers, NULL);
    for (index = 0U; index < count; ++index) {
        if (!llm_equal_ci(server, servers[index].name)) continue;
        if (llm_equal_ci(servers[index].transport, "http") ||
            llm_equal_ci(servers[index].transport, "sse")) {
            if (!llm_mcp_http_target_valid(servers[index].target)) return 1;
                             ...
                             size_t output_size)
{
    ...
    return llm_mcp_exec_all_base(config, arguments,
                                   ...
                                   size_t output_size)
{
    ...
    if (!m258_mcp_net_gate(config, arguments, detail, sizeof(detail)))
        return m258_net_refusal(output, output_size, detail);
    return llm_mcp_exec_transport_base(config, arguments,
                          ...
                          llm_mcp_result *result)
{
    ...
    m258_bridge_detail[0] = '\0';
    ok = llm_mcp_run_result_base(config, arguments,
        stdio_executor, http_executor, sse_executor, context, result);
    if (ok && result->status == LLM_MCP_RES_FAILED &&
        m258_bridge_detail[0] != '\0') {
                          ...
                          char *output, size_t output_size)
{
    ...
    llm_auto_note_tool();
    if (!llm_mcp_run_result(
            getenv("OVMS_AGENT_MCP_SERVERS"), arguments,
            m258_mcp_stdio_exec, m258_mcp_http_exec,
            m258_mcp_sse_exec, NULL, &result)) return 0;

    if (result.status == LLM_MCP_RES_SUCCESS) status_name = "success";
    else if (result.status == LLM_MCP_RES_FAILED) status_name = "failed";
    ...
    llm_log_event("MCP", event,
                     result.status == LLM_MCP_RES_SUCCESS ? 1 : 2);

    return llm_mcp_record_result(arguments, &result, output, output_size);
}
...
void llm_show_mcp_execute(const char *arguments)
{
    ...
    if (!llm_mcp_agent_call(arguments, output, sizeof(output))) {
        (void)puts(
