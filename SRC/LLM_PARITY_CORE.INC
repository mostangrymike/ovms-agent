...
int openai_lang_policy_text(char *output, size_t output_size)
{
    ...
    written = snprintf(output, output_size,
        ...
        "Preserve an established project status convention, including OpenVMS odd/even success semantics when the project uses it, rather than inventing status bit layouts. "
                "For mixed-language ABI claims, source inspection alone does not prove PIC S9(n) COMP layout, calling ABI, pointer representation, GIVING ABI, or external-name mapping; require compiler documentation or a project-demonstrated working interface. "
        "Do not assume Unix trailing underscores or that source spelling/case proves the linker-visible OpenVMS external symbol. "
