...
int main(void)
{
    ...
    if (!openai_gh_run_text("status", "", mock_exec, NULL,
    ...
    if (!openai_gh_run_text("fetch", "origin main", mock_exec, NULL,
                            output, sizeof(output)) ||
        !has(output, "mock fetch origin main")) {
        (void)puts("M250 failed: fetch execution.");
        return 2;
    }
    if (!openai_gh_run_text("clone", "https://github.com/openai/codex.git TEST",
                            mock_exec, NULL, output, sizeof(output)) ||
        !has(output, "mock clone")) {
        (void)puts("M250 failed: GitHub clone validation.");
        return 2;
    }
    if (openai_gh_run_text("clone", "https://example.com/nope.git TEST",
                           mock_exec, NULL, output, sizeof(output))) {
        (void)puts("M250 failed: non-GitHub clone accepted.");
        return 2;
    }
    if (!openai_gh_run_text("push", "origin main", mock_exec, NULL,
                            output, sizeof(output)) ||
    ...
    if (!openai_gh_run_text("issues", "list open", mock_exec, NULL,
                            output, sizeof(output)) ||
        !has(output, "mock issues list open")) {
        (void)puts("M250 failed: issue bridge dispatch.");
        return 2;
    }
    if (!openai_gh_run_text("pr", "list", mock_exec, NULL,
                            output, sizeof(output)) ||
