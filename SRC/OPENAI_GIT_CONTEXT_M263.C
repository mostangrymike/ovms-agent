...
static int m263_git_head_copy(const char *path)
{
    ...
    return fopen(M263_GIT_HEAD_TMP, "r") != NULL;
}
                             ...
                             char *output, size_t output_size)
{
    ...
    output[0] = '\0';
    if (!openai_git_rms_copy(path, M263_GIT_RMS_TMP)) {
        return 0;
    }
...
static int m263_git_modified_path(const char *line,
                                  char *path, size_t path_size)
{
    const char *start;
    size_t length;

    if (line == NULL || path == NULL || path_size == 0U) {
        return 0;
    }

    if (!((line[0] == 'M' || line[1] == 'M') && line[2] == ' ')) {
        return 0;
    }
...
static int m263_git_rms_diff(void)
{
    ...
    while (*cursor != '\0') {
        ...
        next = strchr(cursor, '\n');
        if (m263_git_modified_path(cursor, path, sizeof(path)) &&
            m263_git_one_diff(path, one, sizeof(one))) {
            one_len = strlen(one);
            if (used != 0U) {
...
int openai_git_refresh(const agent_state *state)
{
    ...
    base_ok = openai_git_refresh_base(state);
