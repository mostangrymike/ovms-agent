...
int main(void)
{
    ...
    if (!require_true(
            llm_mcp_exec_all_text(config, "issues get 42",
    ...
    if (!require_true(
            llm_mcp_exec_all_text(config, "stream watch once",
                fake_stdio, fake_http, fake_sse, &probe,
                output, sizeof(output)) &&
            strstr(output, "Status:     success") != NULL &&
            probe.sse_calls == 1,
    ...
    if (!require_true(
            llm_mcp_exec_all_text(config, "stream watch twice",
                fake_stdio, fake_http, fake_sse, &probe,
                output, sizeof(output)) &&
            strstr(output, "default deny") != NULL &&
            probe.sse_calls == 1,
    ...
    if (!require_true(
            llm_mcp_exec_all_text(config, "docs search RMS",
                fake_stdio, fake_http, fake_sse, &probe,
                output, sizeof(output)) &&
