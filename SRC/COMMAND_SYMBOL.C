...
void command_unused_detail(agent_state *state,
                           const char *arguments)
{
    const char *symbol;
    ...
    symbol = command_symbol_argument(
        arguments,
        "Usage: UNUSED/DETAIL symbol"
    );

    if (symbol != NULL) {
        symbol_unused_detail(
            state,
            symbol
        );
    }
...
void command_rename(agent_state *state,
                    const char *arguments)
{
    char *cursor;
    char *old_name;
    ...
    if (arguments == NULL || *arguments == '\0') {
        (void)puts("Usage: RENAME old_name new_name");
        return;
    }
...
void command_rename_verify(agent_state *state,
                           const char *arguments)
{
    char *cursor;
    char *old_name;
    ...
    if (arguments == NULL || *arguments == '\0') {
        (void)puts(
            "Usage: RENAME/VERIFY old_name new_name"
        );
        return;
    }
...
void command_move_function_apply(
    agent_state *state,
    const char *arguments)
{
    char *cursor;
    ...
    if (arguments == NULL || *arguments == '\0') {
        (void)puts(
            "Usage: MOVE/FUNCTION/APPLY "
            "function_name destination.c"
        );
        return;
    }
    ...
    (void)symbol_move_function_apply(
        state,
        symbol,
        destination
    );
