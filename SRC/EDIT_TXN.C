...
static char *edit_txn_copy_text(const char *text)
{
    char *copy;
    size_t length;
...
static int edit_txn_path_valid(const char *path)
{
    if (path == NULL || *path == '\0') {
        return 0;
    }
...
static int edit_txn_versioned_path(const char *path)
{
    const char *semi;
    const char *cursor;
...
static size_t edit_txn_dir_length(const char *path)
{
    const char *split;
    const char *other;
    ...
    return split == NULL ? 0U : (size_t)(split - path) + 1U;
}

static int edit_txn_same_dir(const char *left, const char *right)
{
    size_t left_len;
    size_t right_len;
    size_t index;

    left_len = edit_txn_dir_length(left);
    right_len = edit_txn_dir_length(right);
...
static int edit_txn_find(const edit_txn *transaction, const char *path)
{
    ...
    for (index = 0U; index < transaction->file_count; ++index) {
        if (edit_txn_path_equal(transaction->files[index].path, path)) {
            return (int)index;
        }
    }
...
static int edit_txn_make_hold(edit_txn_file *file)
{
    ...
    prefix_length = split == NULL ? 0U : (size_t)(split - file->original_spec) + 1U;
    ...
    for (tries = 0U; tries < 1024U; ++tries) {
        ...
        if (written < 0 || (size_t)written >= sizeof(file->held_spec)) {
            file->held_spec[0] = '\0';
            return 0;
        }
    ...
    const char *replacement_text)
{
    ...
    if (transaction == NULL || !transaction->active ||
        transaction->committed || !edit_txn_path_valid(path) ||
    ...
    copy = edit_txn_copy_text(replacement_text);
    if (copy == NULL) {
        return 0;
    }
...
int edit_txn_add_delete(edit_txn *transaction, const char *path)
{
    edit_txn_file *file;
    unsigned int index;

    if (transaction == NULL || !transaction->active ||
        transaction->committed || !edit_txn_versioned_path(path)) {
        return 0;
    }
...
int edit_txn_write(edit_txn *transaction)
{
    ...
    for (index = 0U; index < transaction->file_count; ++index) {
        ...
        if (!rms_replace_text_file(file->path, file->replacement_text)) {
...
int edit_txn_commit(edit_txn *transaction)
{
    ...
    if (transaction == NULL || !transaction->active || transaction->committed) {
        return 0;
    }
    ...
    for (index = 0U; index < transaction->file_count; ++index) {
        ...
        if (file->is_rename && file->written) {
            file->written = 0U;
            file->held_spec[0] = '\0';
            continue;
        }
        ...
        if (file->held_spec[0] == '\0' || remove(file->held_spec) != 0) {
            return 0;
        }
    ...
    while (index > 0U) {
        ...
        if (file->existed_before) {
            if (file->original_text == NULL) {
                ...
                for (;;) {
                    if (!edit_txn_get_spec(file->path, current_spec,
                                           sizeof current_spec)) {
                        success = 0;
                        break;
                    }
...
void edit_txn_dispose(edit_txn *transaction)
{
    ...
    for (index = 0U; index < transaction->file_count; ++index) {
        ...
        free(transaction->files[index].original_text);
        transaction->files[index].original_text = NULL;
    }
