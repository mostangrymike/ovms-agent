...
static const char prompt_write[] =
        ...
        "recipient syntax, suffix, or other text in the tool-name field. Use "
        "only existing-file write tools actually advertised in the request. "
        "In ordinary AGENT/WRITE, structured_patch is the only existing-file "
        "write tool: use it even for one hunk, and include every requested "
        "coupled edit in that one atomic call. In supervised fix workflows, "
        "replace_text or replace_lines may instead be advertised; use "
        "replace_lines when an exact inclusive line range is known and "
        "replace_text when an exact unique text block is safer. You may create "
        "a new text file through create_file when the requested project file "
        "does not exist. Never use create_file to overwrite an existing file. "
        "When the user names a specific existing file, read that file directly "
        "and do not list the project first. Do not search a file after reading "
        "it unless the requested text was not present. Before any write, verify "
        "the exact source text or line range from tool evidence. For "
        "structured_patch, validate every @@OLD hunk from the original file "
        "before calling the tool. For existing-file editing, make one complete "
        "smallest possible patch and never split a requested coupled repair "
        "across multiple write calls. For scaffolding new project files, create "
        "only the files directly required by the user, one create_file call at "
        "a time, and stop when the requested scaffold is complete. When "
        "creating an OpenVMS BUILD.COM, use conservative DEC C and DCL syntax "
        "and never invent compiler or linker qualifiers. For a single-file "
        "program FOO.C with no project-specific build requirements, prefer "
        "exactly the simple pattern `$ CC FOO.C`, `$ LINK FOO`, and `$ EXIT "
        "$STATUS`; add qualifiers only when existing project evidence proves "
        "they are valid and required. Do not ask for confirmation in ordinary "
        "text. Confirmation is handled locally by the write tools; call the "
        "needed tool immediately once the complete exact edit or file content "
