...
static char *m209_build_hook(int *build_status)
{
    ++build_call_count;
    ...
    if (build_call_count == 1) {
        *build_status = 2;
        return copy_text(
            ...
            "%CC-E-UNDECLARED, identifier M209_INITIAL is undefined.\n"
        );
    }
    ...
    if (build_call_count == 2) {
        *build_status = 2;
        return copy_text(
            "OpenVMS build status: 2 (failure)\n\n"
            "%CC-E-UNDECLARED, identifier M209_ATTEMPT1 is undefined.\n"
        );
    }
    ...
    if (second_rebuild_succeeds) {
        *build_status = 1;
        return copy_text(
            ...
            "Controlled M209 second repair rebuild passed.\n"
        );
    }
    ...
    *build_status = 2;
    return copy_text(
        "OpenVMS build status: 2 (failure)\n\n"
        "%CC-E-UNDECLARED, identifier M209_ATTEMPT2 is undefined.\n"
    );
}
...
static const char repair_plan1[] =
    "Files to modify\n"
    "TEST/M209_TARGET_A.TMP\n"
    "TEST/M209_TARGET_B.TMP\n"
    "\n"
    "operation_count=2\n"
    "BEGIN_OPERATION\n"
    "type=replace_text\n"
    "path=TEST/M209_TARGET_A.TMP\n"
    "old_text=ALPHA\n"
    "new_text=ALPHA_ONE\n"
    "END_OPERATION\n"
    "BEGIN_OPERATION\n"
    "type=replace_text\n"
    "path=TEST/M209_TARGET_B.TMP\n"
    "old_text=BETA\n"
    "new_text=BETA_ONE\n"
    "END_OPERATION\n";
...
static int run_scenario(int second_succeeds)
{
...

    {
        ...
        if (retry_prompt == NULL ||
            strstr(retry_prompt,
                   "Repair the deterministic M209 failed build") == NULL ||
            strstr(retry_prompt, "ALPHA_ONE") == NULL ||
            strstr(retry_prompt, "M209_ATTEMPT1") == NULL ||
            strstr(retry_prompt, "rolled back") == NULL ||
            strstr(retry_prompt,
                   "Do not repeat the same ineffective repair") == NULL) {
            return 0;
        }
    }
...
int main(void)
{
    ...
    if (!run_scenario(1)) {
        (void)puts(
            "M209 failed: second-attempt success scenario failed."
        );
        cleanup();
        return EXIT_FAILURE;
    }
    ...
    if (!run_scenario(0)) {
        (void)puts(
            "M209 failed: two-attempt rollback scenario failed."
        );
        cleanup();
        return EXIT_FAILURE;
    }
    ...
    cleanup();
    (void)puts("Context-aware second-attempt AGENT/REPAIR test passed.");
    return EXIT_SUCCESS;
}
