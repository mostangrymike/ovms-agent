...
int openai_image_info(const char *path, openai_image_meta *meta)
{
    FILE *file;
...
int openai_image_write_data(FILE *output,
                            const char *path,
                            openai_image_meta *meta)
{
    FILE *file;
    openai_image_meta local;
    ...
    if (output == NULL || !openai_image_info(path, &local)) return 0;
